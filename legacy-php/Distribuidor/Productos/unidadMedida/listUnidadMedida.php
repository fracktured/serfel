...
$lista = new Lista();
$listaUniMed = $lista->getListaUnidadMedida("");
?>

<div id="listProductos" class="">

    <button id="ingUnidadMedida">Ingresar Nuevo Unidad Medida</button>
    <br /><br />

    <table id="tablaUnidadMedida" cellpadding="0" cellspacing="0" border="0" class="display">
        <thead>
            <tr>
                <th>Nombre</th>
                <th>Descripcion</th>
        ...
        <tbody>
            ...
            while ($i <= $lista->getTotalRegistros()) {
                echo "<tr>
                          <input type='hidden' id='" . $listaUniMed[$i]->getId_UM() . "-UnMe'
                                               value='" . $listaUniMed[$i]->getNom_UM() . "'>
                          <td>" . $listaUniMed[$i]->getNom_UM() . "</td>
                          <td>" . $listaUniMed[$i]->getDesc_UM() . "</td>
                          <td class='linkMod'>
                              <a class='linkMod' href='javascript:modUnidadMedida(" . $listaUniMed[$i]->getId_UM() . ")'
                                                 title='Modificar'></a></td>
                          <td class='linkElim'>
                              <a class='linkElim' href='javascript:elimUnidadMedida(" . $listaUniMed[$i]->getId_UM() . ")'
                                                  title='Eliminar'></a></td>
                      </tr>";
                $i++;
            }
...
<div id="popUpElim" title="Eliminación de Unidad Medida">
    <input type="hidden" id="idUnidadMedidaElim" value="" />

    <p id="popUpElimMensaje" class="popUp">
        ¿Está seguro que desea eliminar a <b><span id="nomUnidadMedidaElim"></span></b>?
