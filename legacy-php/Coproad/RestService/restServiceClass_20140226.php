...
class restServiceClass {
	...
	private function executeGet($requestBody) {
		...
		if(strcmp($this->url,"login") == 0 ){
			...
			$this->getProductosPedido($requestBody);
                ...
                } else if(strcmp($this->url, "getPrecioProductosActualizarBD") == 0) {
                    $this->getPrecioProductosActualizarBD($requestBody);
		} else{
			$obj = new stdClass();
	...
	private function getPedidos($requestBody){
		...
		if(json_decode($requestBody) == null){
			...
			$query = "SELECT p.id_pedido,p.fecha_pedido,
					 p.id_local_cliente, lc.tope_venta, p.precio_total,
					 lc.nom_local_cliente, p.id_lista_precio, c.razon_social
				  FROM 30_m_pedido p, 10_m_local_cliente lc, 10_m_cliente c
                                  WHERE DATE(p.fecha_pedido) = CURDATE() 
                                      AND p.id_usuario = ".$idUsuario." 
                                      AND p.id_local_cliente = lc.id_local_cliente
                                      AND p.id_estado = 1
                                      AND lc.rut_cliente = c.rut_cliente
			...
			if($totRes > 0) {
				while ($filaDB = mysql_fetch_assoc($resDB)) {
					...
					$pedido->id_local_cliente = $filaDB["id_local_cliente"];
                                        $pedido->tope_venta = $filaDB["tope_venta"];
					$pedido->precio_total = $filaDB["precio_total"];
					$pedido->nom_local_cliente = $filaDB["nom_local_cliente"];					
                                        $pedido->id_lista_precio = $filaDB["id_lista_precio"];	
                                        $pedido->razon_social = $filaDB["razon_social"];	
					...
					if($totRes > 0) {
						while ($filaDB = mysql_fetch_assoc($resDB)) {
							...
							$precioProducto->max_porcen_desc  = $filaDB["max_porcen_desc"];
                                                        ...
                                                        if($precioProducto->impuesto == 1 || $precioProducto->impuesto == 4 || $precioProducto->impuesto == 5) {
	...
	private function getProducto($requestBody){
        ...
        private function getProductosActualizarBD($requestBody){
            ...
            //$lista = new Lista();
            //$listasPrecio = $lista->getListaPrecios("../");
            ...
            while (($filaDB = mysql_fetch_assoc($resDB))) {
                /*foreach ($listasPrecio as $listaPrecio) {
                    $precioProducto = new PrecioProducto("../", $listaPrecio->getIdListaPrecio(), $filaDB["id_producto"], $searcBy);
                    
                    $obj = object_extractor::get_vars($precioProducto);
                    $objArray[$i] = $obj;
                    $i++;
                }*/
                $prod = new PrecioProducto("../", 1, $filaDB["id_producto"], $searcBy);
                $obj = array();
                
                $obj["id_producto"]         = $prod->getIdProducto();
                $obj["nom_producto"]        = $prod->getNomProducto();
                $obj["nom_marca"]           = $prod->getNomMarca();
                $obj["nom_UM"]              = $prod->getNomUM();
                $obj["cantidad_stock"]      = $prod->getCantidadStock();
                $obj["cantidad_pedida"]     = $prod->getCantidadPedida();
                $obj["cantidad_disponible"] = $prod->getCantidadDisponible();
                $obj["cod_serfel"]          = $prod->getCodSerfel();
        ...
        private function getPrecioProductosActualizarBD($requestBody){
            header("Content-Type: application/json");
            ...
            while (($filaDB = mysql_fetch_assoc($resDB))) {
                foreach ($listasPrecio as $listaPrecio) {
                    ...
                    $prod = new PrecioProducto("../", $listaPrecio->getIdListaPrecio(), $filaDB["id_producto"], $searcBy);
                    $obj = array();

                    $obj["id_lista_precio"] = $listaPrecio->getIdListaPrecio();
                    $obj["id_producto"]     = $prod->getIdProducto();
                    $obj["precio_base"]     = $prod->getPrecioBase();
                    $obj["precio_venta"]    = $prod->getPrecioVenta();
                    $obj["porcen_desc"]     = $prod->getPorcenDesc();
                    $obj["precio_neto"]     = $prod->getPrecioNeto();
                    $obj["max_porcen_desc"] = $prod->getMaxPorcenDesc();
                    $obj["imp_iaba"]        = $prod->getImpIaba();
                    $obj["imp_espec"]       = $prod->getImpEspec();
			...
			while (($filaDB = mysql_fetch_assoc($resDB)) && $i <= 14) {
				$listaRutas[$i] = new LocalCliente($filaDB["id_local_cliente"], $filaDB["nom_local_cliente"],
						...
						$filaDB["apell_mat_contacto"], $filaDB["telefono_contacto"], $filaDB["razon_social"]);
                                ...
                                $listaRutas[$i]->setTopeVenta($filaDB["tope_venta"]);
                                $listaRutas[$i]->setTopeCredito($filaDB["tope_credito"]);
                                $listaRutas[$i]->setRutCliente($filaDB["rut_cliente"]);
                                $listaRutas[$i]->setDVCliente($filaDB["dv_cliente"]);
				$i++;
			}
			mysql_close($db);
			if($listaRutas != null){
	...
	private function addPedido($pedido){
		...
		$query = "INSERT INTO 30_m_pedido (id_pedido,fecha_pedido,id_local_cliente,dia_ruta,id_forma_pago,tiempo,precio_total,id_usuario,id_lista_precio,id_estado) VALUES ".
				"(".$idPedido.", NOW(),".$pedido->id_local_cliente.",".$pedido->dia_ruta.",".$pedido->id_forma_pago.",0,".$pedido->precio_total.",".$pedido->id_usuario.",".$pedido->id_lista_precio.",1)";
	...
	private function editLocalCliente($requestBody){
		if(json_decode($requestBody) == null){
			...
			$resultado = $updLocalCliente->modLocalClienteAndroid($lc->idLocalCliente, $lc->localCliente, $lc->direccionLocalCliente, $lc->fonoLocalCliente, $lc->emailLocalCliente, $lc->nomContacto, $lc->apPaterno, 
