import { mysqlTable, AnyMySqlColumn, foreignKey, int, varchar, datetime, index, date, decimal, smallint, tinyint, primaryKey } from "drizzle-orm/mysql-core"
...
// 8. anio JS property name used for the "año" column in cierre tables — "ñ" is valid JS but the
//    generated compound key reference used table."año" which is invalid object-member syntax;
//    property renamed to anio while SQL column name stays "año".
// 9. AnyMySqlColumn return type added to all .references() callbacks to break forward-reference circularity.
// 10. Removed unused `mysqlSchema` import that drizzle-kit included by default.
...
export const t10MCliente = mysqlTable("10_m_cliente", {
	...
	idListaPrecio: int("id_lista_precio").default(1).notNull().references((): AnyMySqlColumn => t40MListaPrecio.idListaPrecio, { onDelete: "restrict", onUpdate: "restrict" } ),
	idUsuarioMod: int("id_usuario_mod").notNull(),
	ultFechaMod: datetime("ult_fecha_mod", { mode: 'string'}).notNull(),
	idEstado: int("id_estado").default(1).notNull().references((): AnyMySqlColumn => t99PEstado.idEstado, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const t10MEmpresa = mysqlTable("10_m_empresa", {
	...
	idUsuarioMod: int("id_usuario_mod").notNull().references((): AnyMySqlColumn => t10MUsuario.idUsuario, { onDelete: "restrict", onUpdate: "restrict" } ),
	ultFechaMod: datetime("ult_fecha_mod", { mode: 'string'}).notNull(),
	idEstado: int("id_estado").default(1).notNull().references((): AnyMySqlColumn => t99PEstado.idEstado, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const t10MLocalCliente = mysqlTable("10_m_local_cliente", {
	...
	rutCliente: int("rut_cliente").notNull().references((): AnyMySqlColumn => t10MCliente.rutCliente, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const t10MUsuario = mysqlTable("10_m_usuario", {
	...
	idTipoUsuario: int("id_tipo_usuario").notNull().references((): AnyMySqlColumn => t10PTipoUsuario.idTipoUsuario, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const t20MPorcion = mysqlTable("20_m_porcion", {
	...
	idProducto: int("id_producto").notNull().references((): AnyMySqlColumn => t20MProducto.idProducto, { onDelete: "cascade", onUpdate: "restrict" } ),
	...
	idUsuario: int("id_usuario").notNull().references((): AnyMySqlColumn => t10MUsuario.idUsuario, { onDelete: "cascade", onUpdate: "restrict" } ),
	idEstado: int("id_estado").notNull().references((): AnyMySqlColumn => t99PEstado.idEstado, { onDelete: "cascade", onUpdate: "restrict" } ),
},
(table) => [
...
export const t20MProducto = mysqlTable("20_m_producto", {
	...
	idTipoProducto: int("id_tipo_producto").notNull().references((): AnyMySqlColumn => t20PTipoProducto.idTipoProducto, { onDelete: "restrict", onUpdate: "restrict" } ),
	idMarca: int("id_marca").notNull().references((): AnyMySqlColumn => t20PMarca.idMarca, { onDelete: "restrict", onUpdate: "restrict" } ),
	idUm: int("id_UM").notNull().references((): AnyMySqlColumn => t20PUnidadMedida.idUm, { onDelete: "restrict", onUpdate: "restrict" } ),
	idUsuarioMod: int("id_usuario_mod").notNull().references((): AnyMySqlColumn => t10MUsuario.idUsuario, { onDelete: "restrict", onUpdate: "restrict" } ),
	ultFechaMod: datetime("ult_fecha_mod", { mode: 'string'}).notNull(),
	idEstado: int("id_estado").default(1).notNull().references((): AnyMySqlColumn => t99PEstado.idEstado, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const t30MPedido = mysqlTable("30_m_pedido", {
	...
	idLocalCliente: int("id_local_cliente").notNull().references((): AnyMySqlColumn => t10MLocalCliente.idLocalCliente, { onDelete: "restrict", onUpdate: "restrict" } ),
	...
	idUsuario: int("id_usuario").default(5).notNull().references((): AnyMySqlColumn => t10MUsuario.idUsuario, { onDelete: "restrict", onUpdate: "restrict" } ),
	idListaPrecio: int("id_lista_precio").notNull().references((): AnyMySqlColumn => t40MListaPrecio.idListaPrecio, { onDelete: "restrict", onUpdate: "restrict" } ),
	idEstado: int("id_estado").notNull().references((): AnyMySqlColumn => t99PEstado.idEstado, { onDelete: "restrict", onUpdate: "restrict" } ),
},
(table) => [
...
export const t30MProductoPedido = mysqlTable("30_m_producto_pedido", {
	idPedido: int("id_pedido").notNull().references((): AnyMySqlColumn => t30MPedido.idPedido, { onDelete: "restrict", onUpdate: "restrict" } ),
	idProducto: int("id_producto").notNull().references((): AnyMySqlColumn => t20MProducto.idProducto, { onDelete: "restrict", onUpdate: "restrict" } ),
	cantidad: decimal("cantidad", { precision: 18, scale: 3 }).notNull(),
...
export const t40MNotaCredito = mysqlTable("40_m_nota_credito", {
	...
	idVenta: int("id_venta").notNull().references((): AnyMySqlColumn => t40MVenta.idVenta, { onDelete: "restrict", onUpdate: "restrict" } ),
	...
	idUsuario: int("id_usuario").notNull().references((): AnyMySqlColumn => t10MUsuario.idUsuario, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const t40MProdNotaCredito = mysqlTable("40_m_prod_nota_credito", {
	idNotaCredito: int("id_nota_credito").notNull().references((): AnyMySqlColumn => t40MNotaCredito.idNotaCredito, { onDelete: "restrict", onUpdate: "restrict" } ),
	idProducto: int("id_producto").notNull().references((): AnyMySqlColumn => t20MProducto.idProducto, { onDelete: "restrict", onUpdate: "restrict" } ),
	cantidad: decimal("cantidad", { precision: 18, scale: 3 }).notNull(),
...
export const t40MVenta = mysqlTable("40_m_venta", {
	...
	idListaPrecio: int("id_lista_precio").notNull().references((): AnyMySqlColumn => t40MListaPrecio.idListaPrecio, { onDelete: "restrict", onUpdate: "restrict" } ),
	idUsuarioVenta: int("id_usuario_venta").notNull().references((): AnyMySqlColumn => t10MUsuario.idUsuario, { onDelete: "restrict", onUpdate: "restrict" } ),
	iva: int("iva").default(0).notNull(),
	iaba: int("iaba").default(0).notNull(),
	espec: int("espec").default(0).notNull(),
	subTotal: int("sub_total").default(0).notNull(),
	...
	idTipoDoctoEmitido: int("id_tipo_docto_emitido").notNull().references((): AnyMySqlColumn => t10PTipoDocto.idTipoDocto, { onDelete: "restrict", onUpdate: "restrict" } ),
	rutEmpresa: int("rut_empresa").notNull().references((): AnyMySqlColumn => t10MEmpresa.rutEmpresa, { onDelete: "restrict", onUpdate: "restrict" } ),
	rutCliente: int("rut_cliente").notNull().references((): AnyMySqlColumn => t10MCliente.rutCliente, { onDelete: "restrict", onUpdate: "restrict" } ),
	idLocalCliente: int("id_local_cliente").notNull(),
	idFormaPago: int("id_forma_pago").default(0).notNull(),
...
export const t50MBodega = mysqlTable("50_m_bodega", {
	...
	idTipoBodega: int("id_tipo_bodega").notNull().references((): AnyMySqlColumn => t50PTipoBodega.idTipoBodega, { onDelete: "restrict", onUpdate: "restrict" } ),
	idUsuarioMod: int("id_usuario_mod").notNull().references((): AnyMySqlColumn => t10MUsuario.idUsuario, { onDelete: "restrict", onUpdate: "restrict" } ),
	ultFechaMod: datetime("ult_fecha_mod", { mode: 'string'}).notNull(),
	idEstado: int("id_estado").notNull().references((): AnyMySqlColumn => t99PEstado.idEstado, { onDelete: "restrict", onUpdate: "restrict" } ),
},
...
export const t50MCierreMensualBodega = mysqlTable("50_m_cierre_mensual_bodega", {
	...
	mes: int("mes").notNull(),
	idBodega: int("id_bodega").notNull().references((): AnyMySqlColumn => t50MBodega.idBodega, { onDelete: "restrict", onUpdate: "restrict" } ),
	idUsuarioCierre: int("id_usuario_cierre").notNull().references((): AnyMySqlColumn => t10MUsuario.idUsuario, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const t50MProductoRecepcion = mysqlTable("50_m_producto_recepcion", {
	idRecepcion: int("id_recepcion").notNull().references((): AnyMySqlColumn => t50MRecepcionCompra.idRecepcion, { onDelete: "restrict", onUpdate: "restrict" } ),
	idProducto: int("id_producto").notNull().references((): AnyMySqlColumn => t20MProducto.idProducto, { onDelete: "restrict", onUpdate: "restrict" } ),
	cantidad: decimal("cantidad", { precision: 18, scale: 3 }).notNull(),
...
export const t50MRecepcionCompra = mysqlTable("50_m_recepcion_compra", {
	idRecepcion: int("id_recepcion").notNull(),
	rutProveedor: int("rut_proveedor").notNull().references((): AnyMySqlColumn => t70MProveedor.rutProveedor, { onDelete: "restrict", onUpdate: "restrict" } ),
	rutEmpresa: int("rut_empresa").notNull(),
	idTipoDocto: int("id_tipo_docto").notNull().references((): AnyMySqlColumn => t10PTipoDocto.idTipoDocto, { onDelete: "restrict", onUpdate: "restrict" } ),
	numDocto: int("num_docto").notNull(),
	fechaEmisionDocto: datetime("fecha_emision_docto", { mode: 'string'}).notNull(),
	idBodega: int("id_bodega").notNull().references((): AnyMySqlColumn => t50MBodega.idBodega, { onDelete: "restrict", onUpdate: "restrict" } ),
	idUsuarioRecepcion: int("id_usuario_recepcion").notNull().references((): AnyMySqlColumn => t10MUsuario.idUsuario, { onDelete: "restrict", onUpdate: "restrict" } ),
	idEstado: int("id_estado").notNull().references((): AnyMySqlColumn => t99PEstado.idEstado, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const t60MPago = mysqlTable("60_m_pago", {
	...
	idFormaPago: int("id_forma_pago").notNull().references((): AnyMySqlColumn => t10PTipoDocto.idTipoDocto, { onDelete: "cascade", onUpdate: "restrict" } ),
