...
- Product-pricing rows are **upsert-only**. No Activas/Inactivas toggle and no
  restore UI. Price lists can be created, renamed, and removed (DELETE
  inactivates via `id_estado=0`, matching legacy *Eliminar Lista*), but there is
  no restore.
