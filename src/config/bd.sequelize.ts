...
const ProductoPedidoRepo = ProductoPedido.doInit(sequelize);
const ClienteRepo = Cliente.doInit(sequelize);
const ImpuestoRepo = Impuesto.doInit(sequelize);
const StockRepo = Stock.doInit(sequelize);
const EmpresaRepo = Empresa.doInit(sequelize);

// Relaciones de Venta
ProductoVenta.belongsTo(Venta, { as: 'venta', foreignKey: 'idVenta' });
Venta.hasMany(ProductoVenta, { as: 'prodsVenta', foreignKey: 'idVenta' });
...
Pedido.belongsTo(Local, { as: 'local', foreignKey: 'idLocal' });
Local.hasMany(Pedido, { as: 'ventas', foreignKey: 'idLocal' });
Pedido.belongsTo(Usuario, { as: 'vendedor', foreignKey: 'idUsuario' });
Usuario.hasMany(Pedido, { as: 'pedidos', foreignKey: 'idUsuario' });

Local.belongsTo(Cliente, { as: 'cliente', foreignKey: 'rutCliente' });
Cliente.hasMany(Local, { as: 'locales', foreignKey: 'rutCliente' });

Stock.belongsTo(Producto, { as: 'producto', foreignKey: 'idProducto' });
Producto.hasMany(Stock, { as: 'stocks', foreignKey: 'idProducto' });
